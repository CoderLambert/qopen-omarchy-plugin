...
2.4 集中处理不会立即崩溃、但可能损坏用户信任的数据和交互问题：

- 命令和编辑器 argv 使用 POSIX 安全引用回填表单，带空格和单引号的参数不再
  因编辑而改变边界；
- 收藏和删除改用 QML 内部 JSON API，删除具有原生确认层，不再跳转到交互式
  CLI 菜单；
- 新增 `qopen recover`，恢复前完整校验 `config.json.bak`，并将当前损坏目录保存为
  权限为 `0600` 的时间戳快照；
- 新增 `qopen fix-permissions`，`--doctor` 同时审计配置、备份和锁文件权限；
- QML 通过只读 `api catalog` 加载目录，语法正确但不符合 schema 的 JSON 也会进入
  恢复界面，而不是被部分渲染；
- URL 必须包含有效主机且不能嵌入凭据，SSH 目标不能伪装成选项，文本和 argv
  拒绝控制字符；
- PathPicker 为请求分配递增 id，只接受最新响应，并在快速导航时保留最后一次请求；
- QOpen 打开时主动关闭 `omarchy.menu`，避免两个独占键盘焦点 Overlay 同时挂载；
- 剪贴板读取、写入和目标检查失败均返回可见反馈。
...
QML 会传递 `--request-id`，后端在 `result.requestId` 中原样返回。PathPicker 只应用
当前最新请求的响应，旧目录扫描完成后不会覆盖新的导航状态。
